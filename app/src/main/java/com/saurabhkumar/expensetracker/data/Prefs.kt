...
object Prefs {
    ...
    // Theme preferences
    fun getThemeMode(ctx: Context): Int = prefs(ctx).getInt(KEY_THEME_MODE, THEME_SYSTEM)
    ...
    fun setMonthlyBudget(ctx: Context, category: String, amount: Double, month: Int, year: Int) {
        ...
        // Add new budget
        budgets.add(com.saurabhkumar.expensetracker.data.MonthlyBudget(category, amount, month, year))
    ...
    fun getMonthlyBudgets(ctx: Context): List<com.saurabhkumar.expensetracker.data.MonthlyBudget> {
        ...
        return try {
            json.split(";")
                ...
                .map {
                    val parts = it.split(":")
                    com.saurabhkumar.expensetracker.data.MonthlyBudget(
                        category = parts[0],
    ...
    fun getMonthlyBudgetsForMonth(ctx: Context, month: Int, year: Int): List<com.saurabhkumar.expensetracker.data.MonthlyBudget> {
        return getMonthlyBudgets(ctx).filter { it.month == month && it.year == year }
    }

    private fun saveMonthlyBudgets(ctx: Context, budgets: List<com.saurabhkumar.expensetracker.data.MonthlyBudget>) {
    ...
    private fun migrateOldBudgets(ctx: Context): List<com.saurabhkumar.expensetracker.data.MonthlyBudget> {
        ...
        val monthlyBudgets = oldBudgets.map { (category, amount) ->
            com.saurabhkumar.expensetracker.data.MonthlyBudget(category, amount, currentMonth, currentYear)
        }
