    ...
    private fun showPaymentMethodDialog(
        ...
        dialogBinding.chipGroupQuickSelect.setOnCheckedStateChangeListener { _, checkedIds ->
            if (checkedIds.isNotEmpty()) {
                val selectedChip = dialogBinding.chipGroupQuickSelect.findViewById<com.google.android.material.chip.Chip>(
                    checkedIds[0]
                )
        ...
        dialogBinding.btnSave.setOnClickListener {
            ...
            val isDuplicate = paymentMethods.any { method ->
                method.equals(methodName, ignoreCase = true) && (position == -1 || paymentMethods.indexOf(method) != position)
            }
    ...
    private fun showCategoryDialog(title: String, buttonText: String, existingCategory: String?, position: Int) {
        ...
        commonCats.forEach { c ->
            val chip = com.google.android.material.chip.Chip(this).apply {
                ...
                val color = com.saurabhkumar.expensetracker.ui.CategoryUtils.getColorForCategory(
                    this@SettingsModernActivity,
                    c
                )
                chipBackgroundColor = android.content.res.ColorStateList.valueOf(color)
                setTextColor(
                    if ((
                        android.graphics.Color.red(color) * 0.299 + android.graphics.Color.green(color) * 0.587 + android.graphics.Color.blue(
                                color
                            ) * 0.114
                        ) < 150
                    ) {
